...
The stated-subject test is a proxy for meaning (instructions to the reader vs a catalogue of actions). The explanation panel says which reading was chosen; keep that honesty. Auto-detect is the only path — if it guesses wrong, the writer corrects the input or uses 'Report an issue' (there is no manual type override).
...
- **Numbered lists:** preserve numbering when the input is numbered. Numbering is preserved only when the input itself is numbered; the list sub-type (sentence/fragment/stand-alone) is auto-detected. `analyse()` still accepts a `forceNumbered` flag, but the UI no longer exposes a manual 'Numbered' control.
...
- The `analyse(forcedType)` path — forced sentence and numbered formatting, and the limitation notes for forced sentence/fragment mismatches (tests 11 and 11b). The UI no longer offers a manual override, but this logic and these tests remain.
...
Layout: centred single column, `max-width: 960px`, 4px blue keyline across the top. A 1fr 1fr grid holds the input and output cards. Below the grid (in order): the refusal/explanation panel, the report link, the guide, and a source credit.

There is no manual list type selector. The tool relies on automatic detection. A manual override was trialled on 15 June 2026 (a numbered 'Format as' step, then a 'Wrong list type?' correction panel below the explanation) and removed the same day: list type buttons imply the tool can reword items to fit a type, which it cannot. If auto-detect picks the wrong type, the writer corrects the input or uses 'Report an issue'. The `analyse(forcedType)` logic and its limitation notes are kept for the test suite and any future content-aware override (one that offers only the types the current items can become, without rewording).
...
- Re-add a manual list type override (the removed 'Format as' / 'Wrong list type?' panel) without solving the core problem — list type buttons imply the tool can reword items to fit a type, which it cannot. If revisited, offer only the types the current items can actually become (no rewording), or leave it out and rely on auto-detect plus 'Report an issue'.
